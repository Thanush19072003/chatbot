...
def loginaction():
    ...
    if request.method == 'POST':
        ...
        cur.execute('SELECT * FROM signin WHERE username = ? AND password = ?', (username, password))
...
@app.route('/insertvalues',methods= ['GET',"POST"])
def insertvalues():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        cur.execute('SELECT * FROM signin WHERE username = ?', (username,))
        account = cur.fetchone()
        ...
        if account:
            ...
            cur.execute('INSERT INTO signin VALUES (?,?,?,?,?)', (fullname, username, email, password, phone,))
            db.commit()
...
def get_bot_response():
    ...
    if session['step'] == "Description":
        ...
        if session["disease"] in description_list.keys():
            ...
            #return "please visit <a href='" = session["disease"] + "'>  here  </a>"
